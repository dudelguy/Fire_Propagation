...
class active_fire_detection():
    ...
    def find_viirs_for_effis(self, selected_poly, active_fire_data, day_thresh):
        ...
        top_date = pd.to_datetime(selected_poly[self.end_fire]) + pd.Timedelta(days=day_thresh)
        low_date = pd.to_datetime(selected_poly[self.start_fire]) - pd.Timedelta(days=day_thresh)
